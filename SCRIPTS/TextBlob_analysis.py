...
"""
def map_sentiment(score):
    if score >= 0.15:        # Enlarge positive section
        return 'Positive'
    elif score >= -0.15:       # Enlarge neutral section even more
        return 'Neutral'
    else:
        return 'Negative'
...
}).to_csv('../OUTPUTS/new_textblob_metrics.csv', index=False)
...
sns.heatmap(cm, annot=True, fmt='d', cmap='Blues',
            xticklabels=LABELS_3, yticklabels=LABELS_3, ax=axes[0])
axes[0].set_xlabel('Predicted')
axes[0].set_ylabel('True')
axes[0].set_title('New TextBlob — Confusion Matrix')

# Normalized
cm_norm = cm.astype('float') / cm.sum(axis=1, keepdims=True)
sns.heatmap(cm_norm, annot=True, fmt='.2%', cmap='Oranges',
            xticklabels=LABELS_3, yticklabels=LABELS_3, ax=axes[1])
axes[1].set_xlabel('Predicted')
axes[1].set_ylabel('True')
axes[1].set_title('New TextBlob — Normalized Confusion Matrix')

plt.tight_layout()
plt.savefig('new_textblob_confusion_matrix.png', dpi=150, bbox_inches='tight')
plt.show()


# Per-class F1 bar chart

per_class_f1 = f1_score(y_true, y_pred, labels=LABELS_3, average=None, zero_division=0)
colors = ['#D32F2F', '#FFC107', '#2E7D32']

fig, ax = plt.subplots(figsize=(10, 5))
bars = ax.bar(LABELS_3, per_class_f1, color=colors, edgecolor='black')
...
ax.legend()
plt.tight_layout()
plt.savefig('new_textblob_per_class_f1.png', dpi=150, bbox_inches='tight')
plt.show()
...
if DATE_COL in df.columns:
    ...
    plt.tight_layout()
    plt.savefig('new_textblob_sentiment_trend.png', dpi=150, bbox_inches='tight')
    plt.show()

# Error analysis

misclassified = df[df['Sentiment_3'] != df['tb_sentiment']]
print(f'Misclassified: {len(misclassified)}/{len(df)} ({len(misclassified)/len(df)*100:.1f}%)\n')

error_pairs = misclassified.groupby(['Sentiment_3', 'tb_sentiment']).size().reset_index(name='count')
...
df[output_cols].to_csv('new_textblob_predictions.csv', index=False)

print("Ground truth 3-class:")
print(df['Sentiment_3'].value_counts())
print("\nTextBlob predicted:")
print(df['tb_sentiment'].value_counts())

print(df['tb_polarity'].describe())
df['tb_polarity'].hist(bins=50)
